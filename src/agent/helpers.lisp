    ...
    `(let* ((,e!agents ,uuid-or-many)
       ...
       (handler-case (bt:with-timeout (,timeout)
                       (zmq:with-context (,g!context 1)
                         (zmq:with-socket (,mouth-binding ,g!context :sub)
                           (zmq:with-socket (,ear-binding ,g!context :pub)
                             (zmq:setsockopt ,mouth-binding :linger ,e!linger)
                             (zmq:setsockopt ,mouth-binding :reconnect-ivl-max 100)
                             (zmq:setsockopt ,ear-binding :linger ,e!linger)
                             (zmq:setsockopt ,ear-binding :reconnect-ivl-max 100)
                             (mapc #'(lambda (uuid)
                                       (zmq:connect ,mouth-binding (local-ipc-addr uuid :mouth))
                                       (zmq:connect ,ear-binding (local-ipc-addr uuid :ear)))
                                   ,e!agents)
                             (zmq:setsockopt ,mouth-binding :subscribe "")
  ...
  (let ((g!context (gensym "CONTEXT"))
    ...
    `(let ((,e!host ,host)
       ...
       (labels ((,g!process-agent ,arglist ,@forms)
                (,g!discover-agents ()
                  (zmq:with-context (,g!context 1)
                    (zmq:with-socket (,g!seek-sock ,g!context :sub)
                      (zmq:connect ,g!seek-sock (format nil "tcp://~A:~A" ,e!host agent::*common-mouth-port*))
                      (zmq:setsockopt ,g!seek-sock :subscribe "")
