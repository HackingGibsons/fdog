...
(def-fixtures api-functional-fixture
    (:documentation "A fixture that instantiates an api agent."
     :setup (progn
              (start api-runner)
              (unless (wait-for-agent-message (hypervisor-uuid :timeout 60) (msg)
              ...
              (unless (wait-for-agent-message (mongrel2-uuid :timeout 60) (msg)
                        (awhen (getf (getf (getf msg :info) :provides) :servers)
                          (awhen (assoc "control" it :test #'string=)
                            (assoc "api" (cdr it) :test #'string=))))
...
(def-fixtures accounts-agent-fixture
    (:documentation "A fixture that instantiates an api agent."
     :setup (progn
              ...
              (unless (wait-for-agent-message (mongrel2-uuid :timeout 60) (msg)
                               (awhen (getf msg :info)
                                 (and (assoc accounts-agent-uuid (getf it :peers) :test #'string=))))
                (error "Mongrel2 agent didn't peer with accounts agent"))
              (unless (wait-for-agent-message (accounts-agent-uuid :timeout 60) (msg)
                               (awhen (getf msg :info)
